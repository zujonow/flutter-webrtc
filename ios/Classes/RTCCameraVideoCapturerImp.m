...
@interface RTCCameraVideoCapturerImp ()<AVCaptureVideoDataOutputSampleBufferDelegate>
...
@implementation RTCCameraVideoCapturerImp {
...
- (void)dealloc {
  NSAssert(
      !_willBeRunning,
      @"Session was still running in RTCCameraVideoCapturerImp dealloc. Forgot to call stopCapture?");
...
- (void)captureOutput:(AVCaptureOutput *)captureOutput
  ...
  RTCVideoFrame *videoFrame = [[RTCVideoFrame alloc] initWithBuffer:rtcPixelBuffer
    ...
    if (self.createImage) {
        ...
        CVPixelBufferRef pixelBufferr = buffer.pixelBuffer;
        // Extract the image using the buffer
        CIImage *ciImage = [CIImage imageWithCVPixelBuffer:pixelBufferr];
        
        CGFloat rotationAngle = 3 * M_PI / 2;
        // Create a rotation transform
        CGAffineTransform rotationTransform = CGAffineTransformMakeRotation(rotationAngle);
        
        // Create a CIFilter with the CIAffineTransform filter
        CIFilter *affineTransformFilter = [CIFilter filterWithName:@"CIAffineTransform"];
        [affineTransformFilter setValue:ciImage forKey:kCIInputImageKey];
        [affineTransformFilter setValue:[NSValue valueWithBytes:&rotationTransform objCType:@encode(CGAffineTransform)] forKey:@"inputTransform"];
        
        CIImage *rotatedCIImage = [affineTransformFilter outputImage];
        
        CIContext *context = [CIContext context];
        
        CGImageRef cgImage = [context createCGImage:rotatedCIImage fromRect:rotatedCIImage.extent];
        UIImage *uiImage = [UIImage imageWithCGImage:cgImage];
        CGImageRelease(cgImage);
        CGFloat newHeight = self.captureImageHeight;
        CGFloat newWidth = self.captureImageWidth;
        UIImage *resizedImage = [self resizeImage:uiImage newHeight:newHeight newWidth:newWidth];

        // Convert UIImage to base64
        NSData *imageData = UIImageJPEGRepresentation(resizedImage, 1.0);
        self.base64Image = [imageData base64EncodedStringWithOptions:NSDataBase64Encoding64CharacterLineLength];
    }
  [self.delegate capturer:self didCaptureVideoFrame:videoFrame];
}

- (UIImage *)resizeImage:(UIImage *)image newHeight:(CGFloat)newHeight newWidth:(CGFloat)newWidth {
    CGFloat scale;

    if(newHeight == 0.0)
    {
      scale = newWidth / image.size.height;
      newHeight = image.size.width * scale;
    }

    if(newWidth == 0.0){
      scale = newHeight / image.size.width;
      newWidth = image.size.height * scale;
    }
